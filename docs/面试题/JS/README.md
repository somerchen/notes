...
- 扩展运算符用于取出对象中的可便利属性，拷贝到新对象中，与 Object.assign 作用相同
- 数组也和对象一样使用扩展运算符
- rest 参数：把一个分离的序列整合成一个数组/对象，可用于解构，函数参数

## Proxy 可以实现什么功能

Proxy 接收两个参数，一个是要代理的对象，一个是处理器，我们可以在处理器中添加 get 和 set，Vue3 就是根据 Proxy 来实现依赖收集和派发的。
和 Object.defineProperty 相比，Proxy 无需一层层的为每个属性添加代理，一次就能完成所有工作，且可以监听到任何形式的更改，缺点是老旧的浏览器会不支持。

## new 操作符

1. 首先创建一个空对象
2. 把空对象的原型设置为构造函数的 prototype
3. 把函数的 this 指向这个空对象，执行构造函数
4. 判断函数的返回类型，如果是原始值则返回创建的这个对象，如果是引用类型则返回这个引用
