...
`meta` 标签由 `name` 和 `content` 属性定义，用来描述网页的属性，比如网页的作者，网页描述，关键词等等。除了 HTTP 标准固定了一些 `name`，开发者还可以自定义自己的 name。
...
- 进度条、度量器
  ...
  - progress 标签：表示任务的进度。max 表示任务的最大进度，value 表示完成了多少。
...
- async：会在加载完毕后让浏览器尽快执行，如果同时出现 async 和 defer，async 属性起作用。
...
- storage 事件：一个页面更改 localStorage/sessionStorage，其他标签页会触发 storage 事件；
- cookie + setInterval：轮询查看 cookie 里的信息是否改变；
- websocket
- SharedWorker
